"""ACEScct Decode + Save EXR node.

Mirrors logc3_decode.py / logc4_decode.py but uses ACEScct (ceiling ~222.86
linear, with more shadow/midtone precision). Use this with HDR LoRAs trained on
ACEScct targets (the ACEScct Prodigy r128 family: `*_acescct_r128`,
`prodigy_rank128_*`).
"""
import os
...
from ..gear import acescct, exr_io
...
class GearACEScctDecodeSaveEXR:
    """Inverse ACEScct + write float16 EXR.

    Inputs
    - image: HDR LoRA raw output (ACEScct in [0,1]).
    ...
    Outputs
    - hdr_linear: scene-linear HDR (values can exceed 1.0, up to ~222.86).
    ...
    Note: ACEScct's ceiling is ~222.86 linear (≈1 stop below LogC4's ~470).
    Use this node only with LoRAs whose targets were ACEScct-encoded --
    otherwise the EXR values will be wrong.
    """
...
NODE_CLASS_MAPPINGS = {"GearACEScctDecodeSaveEXR": GearACEScctDecodeSaveEXR}
NODE_DISPLAY_NAME_MAPPINGS = {"GearACEScctDecodeSaveEXR": "Gear · ACEScct Decode + Save EXR"}
